 ...
 *
 * @param {String} deviceid. This may a fully-qualified name, of the form
 *  '<domain>:<name>', or it can be a bare name with no domain specified. If
 *  no domain is specified, the default domain will be prepended.
...
Organiq.prototype.getDeviceAuthority = function getDeviceAuthority(deviceid) {
  ...
  try {
    ...
    if (this.localAuthorityIfNoGateway) {
      ...
      // This is what we should eventually do (only have local node act as
      // authority if domain is configured in this.domains).
...
=======
  ...
  for (var deviceid in devices) {
    if (devices.hasOwnProperty(deviceid)) {
      // If we remain authoritative for this device, don't send it to the
      // gateway.
      ...
      if (!this.proxies[deviceid]) {
        this.proxies[deviceid] = [];
>>>>>>> b2f79071234a9ad82531b82ec04555916f89c88f
      }
    }
  }
 ...
 *
 * devices  (devices registered on this node, whether local/remote, authority/no)
 *  id: String
 *  authority: Boolean
 *
 * deviceConnections -
...
<<<<<<< HEAD
  //var clientConnections = [];   // connections by API clients to devices
  //var gatewayConnections = [];  // connections by gateways to devices
  //var registeredGateways = [];
=======
  var clientConnections = [];   // connections by API clients to devices
  var gatewayConnections = [];  // connections by gateways to devices
  var registeredGateways = [];
>>>>>>> b2f79071234a9ad82531b82ec04555916f89c88f
  ...
  Object.keys(this.devices).forEach(function(deviceid) {
    var deviceInfo = {
      deviceid: deviceid,
      authority: self.isAuthoritative(deviceid)
    };
    registeredDevices.push(deviceInfo);
  });

  //Object.keys(this.proxies).forEach(function(deviceid) {
  //  var proxies = self.proxies[deviceid];
  //  var authority = this.isAuthoritative(deviceid);
  //  for(var i=0;i<proxies.length;i++) {
  //    var proxy = proxies[i];
  //    if (authority) {
