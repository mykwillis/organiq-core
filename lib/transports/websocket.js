...
var downstreamCommands = ['GET', 'SET', 'INVOKE', 'DESCRIBE', 'CONFIG'];
...
function WebSocketApi(organiq, options) {
  ...
  return function webSocketApiConnectionHandler(ws) {
    ...
    /**
     * Collection of active device proxies.
     *
     ...
     */
    var proxies = {};   // WebSocketDeviceProxy objects by connid
    ...
    function processMessage(data, flags) {
      ...
      if (isDownstreamCommand(method)) {
        ...
        switch (method) {
          ...
          case 'DESCRIBE':
            ...
            break;
          case 'CONFIG':
            promise = device.config(msg.identifer, msg.value);
            break;
        }
      ...
      // Upstream commands may be received when the remote node is authoritative,
      // or we are authoritative and the device is attached to the remote node.
      if (isUpstreamCommand(method)) {
        if (typeof proxies[msg.deviceid] === 'undefined') {
          var e = 'Invalid upstream command: bad device ID.';
          return sendFailureResponse(msg, e);
        }
        var proxy = proxies[msg.deviceid];
        switch (method) {
          case 'PUT':
            proxy.emit('put', msg.identifier, msg.value);
            break;
          case 'NOTIFY':
            proxy.emit('notify', msg.identifier, msg.value);
            break;
        }
    ...
    handlers['CONNECT'] = function handleConnect(req) {
      ...
      debug('Connected remote device: ' + deviceid);
...
/**
 * Get a proxy for a device for which this gateway is authoritative.
 *
 * This method is called by the local node when a connection request is made
 * for a device for which this gateway is authoritative.
 *
 * The returned device proxy uses the underlying WebSocket transport to
 * communicate device requests with the authoritative node. It also handles
 *
 * @param {String} deviceid
